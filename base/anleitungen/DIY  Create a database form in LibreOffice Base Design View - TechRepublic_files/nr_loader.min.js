...
5E3)},acs:function(a){if(this.plugins_sanitized&&!this.custom_scripts_loaded){this.custom_scripts_loaded=!0;var b;this.debug("Applying custom scripts");for(b in this.options.plugins)this.extend(this.options.plugins[b],a[b]);this.nr_cs_load_time=(new Date).getTime();if(!this.options.browser.msie)try{this.pd()}catch(c){}this.bdr(this.pd,"pd()")}},pd:function(){var a,b,c,d,e,h;for(p in this.options.plugins){a=this.options.plugins[p];this.is_function(a.fph)?(b=a.fph,this.debug("Use custom fph for %s",
p)):(b=this.dfph,this.debug("Use default fph for %s",p));this.is_function(a.fpl)?(c=a.fpl,this.debug("Use custom fpl for %s",p)):(c=this.dfpl,this.debug("Use default fpl for %s",p));this.is_function(a.fkw)?(d=a.fkw,this.debug("Use custom fkw for %s",p)):(d=this.dfkw,this.debug("Use default fkw for %s",p));this.debug("Parsing document for %s",p);e=a.cts.length;a.phs=b.call(this,p);a.pls=c.call(this,p);h={};b=0;for(c=a.pls.length;b<c;b++)h[a.pls[b]]=1;this.is_home_page=Boolean(this.is_defined(window.nr_is_home)?
window.nr_is_home:1<this.get_keys(h).length);a.kws=d.call(this,p);b=e;for(c=a.phs.length;b<c&&b<(1>=c?1:15);++b)this.act(b,p);this.mac()}},dfph:function(a){var b,c,d,e,h,l,g,f=[],j="post-body article-content entry-content entry post-entry post-inner postmeta2 post_content text postcontent single post-header content post art-PostContent jump-link entrytitle wrap blogPost post-footer postfooter commentpage post-footer-line".split(" "),m=["post-footer","postfooter","commentpage","post-footer-line"],
n=["format_teaser","sideContent","ngg-widget"];if(f=this.xgebi("nrelate_"+a+"_placeholder"))return this.debug("Backwards compatibility: using #nrelate_%s_placeholder: %o",a,f),[f];if(0<(f=this.xgebcn("nr_"+a+"_placeholder","div")).length)return this.debug("Using manually located .nr_%s_placeholder: %o",a,f),f;b=0;for(e=j.length;b<e;b++)if(g=this.xgebcn(j[b]),0<(h=g.length)){this.debug("Found candidates with .%s selector",j[b]);for(c=0;c<h;c++){d=0;for(l=n.length;d<l;d++)if(this.xhc(g[c],n[d])){this.debug("%o excluded because of class .%s",
g[c],n[d]);g[c]=!1;break}g[c]&&0<=this.in_array(j[b],m)&&(this.is_defined(g[c].previousSibling)&&1===g[c].previousSibling.nodeType?(this.debug("Asigning previousSibling because of .%s",j[b]),g[c]=g[c].previousSibling):(this.debug("%o excluded because it doesn't have valid previousSibling",g[c]),g[c]=!1));g[c]&&(f[f.length]=g[c])}if(f.length)return this.debug("Document parsed. Found these placeholders %o",f),f}else this.debug("No placeholders found with .%s selector",j[b]);if(f=this.xgebi("nrelate_"+
a+"_backup_placeholder"))return this.debug("Backwards compatibility: using #nrelate_%s_backup_placeholder: %o",a,f),[f];this.debug("No placeholders found");return[]},dfpl:function(a){var b,c,d,e,h,l,g,f,j=[];g=this.options.plugins[a];f=["entry-title","post-title","entry-header"];var m=["h3","h2","h1"];if(1==g.phs.length&&"undefined"!=typeof window.nr_pageurl)return this.debug("Manually defined nr_pageurl"),[{href:window.nr_pageurl}];if(0<g.phs.length&&g.phs[0].getAttribute("data-permalink")){b=0;
for(e=g.phs.length;b<e;b++)j[j.length]={href:this.get_data_attr(g.phs[b],"permalink")};this.debug("Found data-permalink attribute on placeholders");return j}if(1<(j=this.xgeba("a","title","permanent link")).length)return this.debug("Found title='permanent link' permalinks %o",j),j;b=0;for(e=f.length;b<e;b++){c=0;for(h=m.length;c<h;c++){if(0<(l=this.xgebcn(f[b],m[c])).length)for(d=0;d<l.length;d++)if(0<(g=this.xgebtn("a",l[d])).length)j[j.length]=g[0];if(0<j.length)return this.debug("Found permalinks on %s.%s %o",
m[c],f[b],j),j}}if(0<(j=this.xgeba("a","rel","bookmark")).length)return this.debug("Found rel='bookmark' permalinks %o",j),j;if(0<(j=this.xgebcn("permalink","a")).length)return this.debug("Found class='permalink' permalinks %o",j),j;if(0<(f=this.xgebcn("post")).length){b=0;for(e=f.length;b<e;b++)if(0<(g=this.xgebtn("a",f[b])).length)j[j.length]=g[0];if(0<j.length)return this.debug("Found first links inside div.post %o",j),j}if(0<(f=this.options.plugins[a].phs).length){b=0;for(e=f.length;b<e;b++)if(0<
(g=this.xgebtn("a",f[b])).length)j[j.length]=g[0];if(0<j.length)return this.debug("Found first links inside div.post %o",j),j}this.debug("No permalinks found");return[]},dfkw:function(a){var b,c,d,e,h,l=document.title,f=[],r=this.options.plugins[a],j=["entry-title","post-title","entry-header"],m=["h3","h2","h1"];if("undefined"!=typeof window.nr_keywords&&"encodeURI(document.title)"!=window.nr_keywords)return this.debug("Manually defined nr_keywords"),[window.nr_keywords];if(0<r.phs.length&&r.phs[0].getAttribute("data-title")){a=
0;for(b=r.phs.length;a<b;a++)f[f.length]=this.get_data_attr(r.phs[a],"title");this.debug("Found data-title attribute on placeholders");return f}a=0;for(b=j.length;a<b;a++){c=0;for(d=m.length;c<d;c++){if(0<(e=this.xgebcn(j[a],m[c])).length)for(k=0;k<e.length;k++)if(h=e[k].innerText||e[k].textContent,this.is_home_page)f[f.length]=h;else if(0<=l.search(h)){a=0;for(b=r.pls.length;a<b;a++)f[a]=h;break}if(0<f.length)return this.debug("Found keywords on %s.%s %o",m[c],j[a],f),f}}if(!this.is_home_page){a=
0;for(b=r.pls.length;a<b;a++)f[a]=l;this.debug("Single post page. Using document.title as keyword");return f}this.debug("No keywords found");return[]},act:function(a,b,c){var d,e=this.options.plugins[b];c="string"!=typeof c?"":c;d=document.createElement("div");d.id="nrelate_"+b+"_"+a;d.innerHTML=c;this.xac(d,"nrelate");this.xac(d,"nrelate_"+b);this.xac(d,"nr_clear");d.nr_api_called=!1;d.nr_plugin=b;d.nr_viewed=!1;e.phs[a].appendChild(d);e.cts[a]=d},mac:function(){var a,b,c,d,e,h,f;for(a in this.options.plugins)if(b=
this.options.plugins[a],this.is_function(b.should_load)&&!b.should_load.call(this,b))this.debug("Ignore %s API calls because of should_load",a);else{this.debug("Making API calls for %s",a);c=0;for(d=b.cts.length;c<d;c++)if(!(e=b.cts[c]).nr_api_called){if(!this.is_defined(b.pls[c]))if(1==d)b.pls[c]={href:window.location.href};else continue;h=1==b.phs.length?document.title:"";h=this.aurlp(b.api_url,{tag:"nrelate_"+a,domain:this.options.domain,keywords:b.kws[c]||h,url:b.pls[c].href,nr_div_number:c,pr_id:this.get_print_id()});
"referrer"in document&&document.referrer&&(h=this.aurlp(h,{referrer:document.referrer}));this.is_home_page||(e.nr_src_url=b.pls[c].href);h=this.apip(b,h,e);this.nr_parse_time=(new Date).getTime();if("performance"in window){f=this.nr_load_time-window.performance.timing.responseStart;var g=this.nr_cs_load_time-this.nr_load_time,m=this.nr_parse_time-Math.max(window.performance.timing.domInteractive,this.nr_cs_load_time);this.debug("#%s Loading time: %f | CS Loading time: %f | Parse time %f",e.id,f/1E3,
g/1E3,m/1E3);h=this.aurlp(h,{loading_time:f,cs_time:g,parse_time:m})}if(f=this.parse_json(this.get_data_attr(e.parentNode,"options")))h=this.aurlp(h,f);e.nr_api_call_time=(new Date).getTime();this.debug("Making API call for #%s: %o",e.id,h);this.lr(h,e.id+"-api_call");e.nr_api_called=!0;e.nr_api_url=h}}},apip:function(a,b,c){var d,e,h={},f="cssstyle thumbsize ad_place widgetstyle header ad_header widget_id page_type_id page_type geo article_id".split(" "),g=["norelatedposts","ad_num"];this.is_array(this.options.extra_apip.general)&&
(f=f.concat(this.options.extra_apip.general));d=0;for(e=f.length;d<e;d++)null!==a[f[d]]&&this.is_defined(a[f[d]])&&(h[f[d]]=a[f[d]]);this.is_array(this.options.extra_apip.numeric)&&(g=g.concat(this.options.extra_apip.numeric));d=0;for(e=g.length;d<e;d++)isNaN(a[g[d]])||(h[g[d]]=a[g[d]]);h.length!=={}&&(b=this.aurlp(b,h));this.is_function(a.capip)&&(b=a.capip.call(this,b,a,c));return b},sw:function(a,b,c){args=c||{};(c=this.xgebi(b))?(this.debug("Injecting content into #%s container",b),b=this.options.plugins[c.nr_plugin],
this.sctc(b,c,args),c.innerHTML=a,this.sct(b,c,args),c.nr_top_distance=this.xPageY(c)+this.xHeight(c),c.nr_left_distance=this.xPageX(c)+this.xWidth(c),c.nr_widget_initialized_time=(new Date).getTime(),c.nr_count_views=args.count_views||b.count_views,this.bvc()):this.debug("#%s container not found",b)},sctc:function(a,b,c){a.cssstyle=c.cssstyle||c.style||a.cssstyle||a.supported_cssstyles[1];a.cssversion=c.cssversion||0;a.thumbsize=c.thumbsize||c.thumb_size||a.thumbsize||a.supported_thumbsizes[1];a.ad_place=
c.ad_place||a.ad_place||a.supported_ad_places[1];a.widgetstyle=c.widgetstyle||a.widgetstyle||a.supported_widgetstyles[1];a.whats_this_link=c.whats_this_link||a.whats_this_link||!1;a.widget_id=c.widget_id||a.widget_id||"0";a.page_type_id=c.page_type_id||a.page_type_id||"0";a.page_type=c.page_type||a.page_type||"0";a.geo=c.geo||a.geo||"0";a.article_id=c.article_id||a.article_id||"0";if(a.cssstyle)if("custom"==a.cssstyle){c=this.to_slug(this.options.domain);var d=(this.options.debug_mode?this.options.debug_css_api_url:
this.options.css_api_url)+c+".css",d=this.aurlp(d,{v:a.cssversion});this.debug("Loading custom style from %o",d);this.lr(d,"nrelate-"+c+"custom-style",{type:"css",callback:this.fah})}else"none"!=a.cssstyle&&this.lr(this.options.default_stylesheet_url,"nrelate-styles",{type:"css",callback:this.fah});this.bind(window,"load",function(){nRelate.fah()});this.xac(b,"nrelate_"+a.cssstyle);0===a.widgetstyle?this.xac(b,"nr_text"):this.xac(b,"nr_"+a.thumbsize);"Separate"==a.ad_place?this.xac(b,"nr_2col"):this.xac(b,
"nr_1row")},sct:function(a,b,c){this.xac(b,"nr_initializing");if(c.widget_id||a.widget_id)this.xac(b,"widget_id_"+(c.widget_id||a.widget_id)),b.nr_widget_id=c.widget_id||a.widget_id;b.nr_cols_num=c.cols_num||0;this.fh(b);this.ctr(b);this.aa(b);this.pr(b);a.whats_this_link&&this.wtl(b,a,c);this.is_function(a.csct)&&a.csct.call(this,a,b,c);this.xrc(b,"nr_initializing");this.xac(b,"nr_setup")},fh:function(a){if((a=this.xgebi(a))&&!0!=a.nr_fixing_height){var b;if(0!=(b=this.xgebcn("nr_panel","a",a)).length){a.nr_fixing_height=
!0;var c,d,e,h,f;num_rows=num_cols=img_height=link_height=link_y=d=0;row_links=[];tallest=0;_col_counter=_row_counter=1;f=this.xgebtn("img",a);c=0;for(e=f.length;c<e;c++)img_height=Math.max(img_height,this.xHeight(f[c]));this.xrc(a,"rotate");c=0;for(e=b.length;c<e;c++)b[c].style.minHeight=0,b[c].style.transition="none";d=this.xPageY(b[0]);c=0;for(e=b.length;c<e;c++)if(f=b[c],link_height=Math.max(this.xHeight(f),img_height),a.nr_cols_num)row_links[row_links.length]=f,tallest=tallest<link_height?link_height:
tallest,num_cols=a.nr_cols_num;else{link_y=this.xPageY(f);if(d!=link_y){d=0;for(h=row_links.length;d<h&&!this.xhc(a,"nr_text");d++)row_links[d].style.minHeight=tallest+"px";row_links=[];d=link_y;tallest=link_height;row_links[row_links.length]=f}else row_links[row_links.length]=f,tallest=tallest<link_height?link_height:tallest;num_cols=num_cols<row_links.length?row_links.length:num_cols}d=0;for(h=row_links.length;d<h&&!this.xhc(a,"nr_text");d++)row_links[d].style.minHeight=tallest+"px";c=0;for(e=b.length;c<
...
(b=b.parentNode);return b},lr:function(a,b,c){if(!b||!this.xgebi(b)){var d=!1;"string"==typeof c&&(c={type:c});c=this.extend({type:"script",async:!0,custom_js:""},c);switch(c.type){case "script":if(c.async){var e,f,l,d=document.createElement("div");d.style.display="none";b&&(d.id=b);e=document.createElement("iframe");e.frameBorder="0";e.allowTransparency="true";try{document.body.insertBefore(d,document.body.firstChild)}catch(g){c.async=!1,this.lr(a,b,c)}d.appendChild(e);try{e.contentWindow.document.open()}catch(m){f=
"javascript:var d=document.open();d.domain='"+document.domain+"';",e.src=f+"void(0);"}a='<body onload="var d=document;nRelate=window.parent.nRelate;'+c.custom_js+"d.getElementsByTagName('head')[0].appendChild(d.createElement('script')).src='"+a+"';\"></body>";try{l=e.contentWindow.document,l.write(a),l.close()}catch(j){e.src=f+"d.write('"+a.replace(/"/g,'\\"')+"');d.close();"}return d}d=document.createElement("script");d.type="text/javascript";d.src=a;break;case "css":case "link":if(d=document.createElement("link"),
d.type="text/css",d.rel="stylesheet",d.href=a,this.is_function(c.callback)){e=document.createElement("img");var n=this;e.onerror=function(){c.callback.call(n,b,d)};e.src=a}}d&&(b&&(d.id=b),a=this.xgebtn("head")[0]||document.documentElement,a.insertBefore(d,a.firstChild));return d}},xgebcn:function(a,b,c,d){var e=[],f;a=RegExp("(^|\\s)"+a+"(\\s|$)");b=this.xgebtn(b,c);c=0;for(f=b.length;c<f;++c)a.test(b[c].className)&&(e[e.length]=b[c],d&&d(b[c]));return e},xgebtn:function(a,b){var c=null;a=a||"*";
b=this.xgebi(b)||document;if(this.is_defined(b.getElementsByTagName)){if(c=b.getElementsByTagName(a),"*"==a&&(!c||!c.length))c=b.all}else"*"==a?c=b.all:b.all&&b.all.tags&&(c=b.all.tags(a));return c||[]},xgebi:function(a){"string"==typeof a&&(a=document.getElementById?document.getElementById(a):document.all?document.all[a]:null);return a},xgeba:function(a,b,c,d){var e,f,l=[];c=RegExp(c,"i");f=this.xgebtn(a);var g=0;for(e=f.length;g<e;++g)(a=f[g].getAttribute(b))||(a=f[g][b]),"string"===typeof a&&-1!==
a.search(c)&&(l[l.length]=f[g],d&&d(f[g]));return l},xhc:function(a,b){a=this.xgebi(a);return!a||""==a.className?!1:RegExp("(^|\\s)"+b+"(\\s|$)").test(a.className)},xac:function(a,b){if(a=this.xgebi(a)){var c="";a.className.length&&" "!=a.className.charAt(a.className.length-1)&&(c=" ");if(!this.xhc(a,b))return a.className+=c+b,!0}return!1},xrc:function(a,b){if(!(a=this.xgebi(a)))return!1;a.className=a.className.replace(RegExp("(^|\\s)"+b+"(\\s|$)","g"),function(a,b,e){return" "==b&&" "==e?" ":""});
a.className=a.className.replace(/^\s+|\s+$/,"");return!0},xPageY:function(a){var b=0;for(a=this.xgebi(a);a;)this.is_defined(a.offsetTop)&&(b+=a.offsetTop),a=this.is_defined(a.offsetParent)?a.offsetParent:null;return b},xPageX:function(a){var b=0;for(a=this.xgebi(a);a;)this.is_defined(a.offsetLeft)&&(b+=a.offsetLeft),a=this.is_defined(a.offsetParent)?a.offsetParent:null;return b},xHeight:function(a,b){var c,d=0,e=0,f=0,l=0;if(!(a=this.xgebi(a)))return 0;b=isNaN(b)||"number"!==typeof b?-1:0>b?0:Math.round(b);
c=this.is_defined(a.style);if(a==document||"html"==a.tagName.toLowerCase()||"body"==a.tagName.toLowerCase())b=this.xClientHeight();else if(c&&this.is_defined(a.offsetHeight)&&"string"==typeof a.style.height){if(0<=b){"CSS1Compat"==document.compatMode&&(d=this.xGetComputedStyle(a,"padding-top",1),null!==d?(e=this.xGetComputedStyle(a,"padding-bottom",1),f=this.xGetComputedStyle(a,"border-top-width",1),l=this.xGetComputedStyle(a,"border-bottom-width",1)):this.is_defined(a.offsetHeight,a.style.height)&&
(a.style.height=b+"px",d=a.offsetHeight-b));b-=d+e+f+l;if(isNaN(b)||0>b)return;a.style.height=b+"px"}b=a.offsetHeight}else c&&this.is_defined(a.style.pixelHeight)&&(0<=b&&(a.style.pixelHeight=b),b=a.style.pixelHeight);return b},xWidth:function(a,b){var c,d=0,e=0,f=0,l=0;if(!(a=this.xgebi(a)))return 0;b=isNaN(b)||"number"!==typeof b?-1:0>b?0:Math.round(b);c=this.is_defined(a.style);if(a==document||"html"==a.tagName.toLowerCase()||"body"==a.tagName.toLowerCase())b=this.xClientWidth();else if(c&&this.is_defined(a.offsetWidth)&&
"string"==typeof a.style.width){if(0<=b){"CSS1Compat"==document.compatMode&&(d=this.xGetComputedStyle(a,"padding-left",1),null!==d?(e=this.xGetComputedStyle(a,"padding-right",1),f=this.xGetComputedStyle(a,"border-left-width",1),l=this.xGetComputedStyle(a,"border-right-width",1)):this-is_defined(a.offsetWidth,a.style.width)&&(a.style.width=b+"px",d=a.offsetWidth-b));b-=d+e+f+l;if(isNaN(b)||0>b)return;a.style.width=b+"px"}b=a.offsetWidth}else c&&this.is_defined(a.style.pixelWidth)&&(0<=b&&(a.style.pixelWidth=
b),b=a.style.pixelWidth);return b},xClientHeight:function(){var a=0,b=document,c=window;(!b.compatMode||"CSS1Compat"==b.compatMode)&&b.documentElement&&b.documentElement.clientHeight?a=b.documentElement.clientHeight:b.body&&b.body.clientHeight?a=b.body.clientHeight:this.is_defined(c.innerWidth,c.innerHeight,b.width)&&(a=c.innerHeight,b.width>c.innerWidth&&(a-=16));return a},xClientWidth:function(){var a=0,b=document,c=window;(!b.compatMode||"CSS1Compat"==b.compatMode)&&!c.opera&&b.documentElement&&
b.documentElement.clientWidth?a=b.documentElement.clientWidth:b.body&&b.body.clientWidth?a=b.body.clientWidth:this.is_defined(c.innerWidth,c.innerHeight,b.height)&&(a=c.innerWidth,b.height>c.innerHeight&&(a-=16));return a},xDocSize:function(){var a=document.body,b=document.documentElement,c=0,d=0,e=0,f=0,l=0,g=0,m=0,j=0;b&&(c=b.scrollWidth,d=b.offsetWidth,l=b.scrollHeight,g=b.offsetHeight);a&&(e=a.scrollWidth,f=a.offsetWidth,m=a.scrollHeight,j=a.offsetHeight);return{w:Math.max(c,d,e,f),h:Math.max(l,
g,m,j)}},xScrollTop:function(a,b){var c,d=0;if(!this.is_defined(a)||b||a==document||"html"==a.tagName.toLowerCase()||"body"==a.tagName.toLowerCase())c=window,b&&a&&(c=a),c.document.documentElement&&c.document.documentElement.scrollTop?d=c.document.documentElement.scrollTop:c.document.body&&this.is_defined(c.document.body.scrollTop)&&(d=c.document.body.scrollTop);else if((a=this.xgebi(a))&&!isNaN(a.scrollTop)&&"number"==typeof a.scrollTop)d=a.scrollTop;return d},xScrollLeft:function(a,b){var c,d=0;
if(!this.is_defined(a)||b||a==document||"html"==a.tagName.toLowerCase()||"body"==a.tagName.toLowerCase())c=window,b&&a&&(c=a),c.document.documentElement&&c.document.documentElement.scrollLeft?d=c.document.documentElement.scrollLeft:c.document.body&&this.is_defined(c.document.body.scrollLeft)&&(d=c.document.body.scrollLeft);else if((a=this.xgebi(a))&&!isNaN(a.scrollLeft)&&"number"==typeof a.scrollLeft)d=a.scrollLeft;return d},xGetComputedStyle:function(a,b,c){if(!(a=this.xgebi(a)))return null;var d=
"undefined",e=document.defaultView;if(e&&e.getComputedStyle)(a=e.getComputedStyle(a,""))&&(d=a.getPropertyValue(b));else if(a.currentStyle)d=a.currentStyle[this.xCamelize(b)];else return null;return c?parseInt(d)||0:d},xOpacity:function(a,b){var c=this.is_defined(b);if(!(a=this.xgebi(a)))return 2;"string"==typeof a.style.opacity?c?a.style.opacity=b+"":b=parseFloat(a.style.opacity):"string"==typeof a.style.filter?c?a.style.filter="alpha(opacity="+100*b+")":a.filters&&a.filters.alpha&&(b=a.filters.alpha.opacity/
100):"string"==typeof a.style.MozOpacity?c?a.style.MozOpacity=b+"":b=parseFloat(a.style.MozOpacity):"string"==typeof a.style.KhtmlOpacity&&(c?a.style.KhtmlOpacity=b+"":b=parseFloat(a.style.KhtmlOpacity));return isNaN(b)?1:b},xCamelize:function(a){var b,c,d;c=a.split("-");d=c[0];for(a=1;a<c.length;++a)b=c[a].charAt(0),d+=c[a].replace(b,b.toUpperCase());return d},options:{auto_invoke:"undefined"!=typeof window.nr_manual?!window.nr_manual:!0,posts_only:"undefined"!=typeof window.nr_posts_only?window.nr_posts_only:
!1,debug_mode:/.*\?.*nrelate_debug=.*/.test(window.location)||Boolean(window.nr_debug),debug_cs_api_url:"http://staticrepo.nrelate.com/custom-script/1.0/dev/",debug_css_api_url:"http://staticrepo.nrelate.com/custom-style/dev/"},constants:{version:"0.52.0",domain:"undefined"!=typeof window.nr_domain?window.nr_domain:window.location.hostname,cs_api_url:"http://js.nrcdn.com/custom-script/1.0/",css_api_url:"http://css.nrcdn.com/custom-style/",default_stylesheet_url:"http://css.nrcdn.com/custom-style/default.min.css",
tracking_url:"http://t4.nrelate.com/tracking/",remove_url:"http://api4.nrelate.com/rcw_js/0.52.0/nr_removeID.php",views_url:"http://vt.nrelate.com/vt/",print_id:!1,extra_apip:{}},nr_load_time:window.nr_load_time,nr_cs_load_time:0,nr_parse_time:0,is_initialized:!1,custom_scripts_loaded:!1,is_home_page:!1,plugins_sanitized:!1,views_handler_bound:!1,middle_click:!1,clicked_link:!1,is_dom_ready:!1,is_domready_bound:!1,domready_callbacks:{}},q=navigator.userAgent.toLowerCase();f.options.ua=q;f.options.browser=
...
count_views:!1,should_load:function(){return this.is_home_page&&this.options.posts_only?!1:!0},_defaults:{whats_this_link:!1,whats_this_content:"about these links",widget_id:null,page_type_id:null,page_type:null,geo:null,article_id:null}};f.plugin_helpers={related:f.extend({},q,{api_url:"http://api4.nrelate.com/rcw_js/"+f.constants.version+"/",shortname:"rc",fullname:"related"}),popular:f.extend({},q,{api_url:"http://api4.nrelate.com/mpw_wp/"+f.constants.version+"/",loadcounter_url:"http://api4.nrelate.com/mpw_wp/"+
...
this.bind(d,"click",function(){a.fo_animate_close.call(e,a,b);a.fo_animate_open.call(e,a,b,fo_open);s.set("nr_fo_closed","true",7,"/")});a.offset=c.offset||a.offset||1;a.element=c.element||a.element||null;a.hscroll.call(e,null,a,b);this.bind(window,"scroll",function(c){a.hscroll.call(e,c,a,b)})},hscroll:function(a,b,c){var d;if(!nRelate.flyout_show)return!1;a=b.should_open.call(this,b);if("true"==s.get("nr_fo_closed")){if(!(d=this.xgebi("nrelate_flyout_open")))return!1;a?b.fo_animate_open.call(this,
...
e.res))};n.prototype.pause=function(){clearInterval(this.tmr);this.tmr=null};n.prototype.resume=function(a){"undefined"!=typeof this.tmr&&!this.tmr&&(this.t1=(new Date).getTime(),a||(this.t1-=this.et),this.run(!a))};n.prototype.css=function(a,b,c,d,e,h,l){function g(a){a.e.style[a.prop]=Math.round(a.a[0].v)+"px"}this.axes(1);this.a[0].i=f.xGetComputedStyle(a,b,!0);this.a[0].t=c;this.prop=f.xCamelize(b);this.init(a,d,g,g,l,e,h);this.run()};n.prototype.opacity=function(a,b,c,d,e,h){function l(a){f.xOpacity(a.e,
a.a[0].v)}this.axes(1);this.a[0].i=f.xOpacity(a);this.a[0].t=b;this.init(a,c,l,l,h,d,e);this.run()};n.vf=[function(a){return a},function(a){return Math.abs(Math.sin(a))},function(a){return 1-Math.abs(Math.cos(a))},function(a){return(1-Math.cos(a))/2},function(a){return 1-Math.exp(6*-a)},function(a){return-Math.cos(a*Math.PI)/2+0.5}];var s={get:function(a){return(a=document.cookie.match(RegExp("(^|;)\\s*"+a+"=([^;\\s]*)")))&&3<=a.length?unescape(a[2]):null},set:function(a,b,c,d,e,f){if(c){var l=new Date;
l.setTime(l.getTime()+864E5*c)}document.cookie=a+"="+escape(b)+(c?"; expires="+l.toGMTString():"")+"; path="+(d||"/")+(e?"; domain="+e:"")+(f?"; secure":"")},del:function(a,b,c){this.set(a,"",-1,b,c)}};m.prototype.show=function(){var a=m.grey;this.is_opened=!0;a&&(this.dialog.greyZIndex=f.xGetComputedStyle(a,"z-index",1),a.style.zIndex=f.xGetComputedStyle(this.dialog,"z-index",1)-1);this.dialog&&this.center()};m.prototype.center=function(){var a=f.xClientWidth(),b=f.xClientHeight(),c=m.grey;f.is_defined(this.dialog.max_dimensions)||
